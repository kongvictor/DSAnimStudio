...
public class FBXExporter
{
    public static Scene ExportToScene(HavokAnimationData anim)
    {
        ...
        if (anim.RootMotion != null)
        {
            ...
            for (int f = 0; f < anim.FrameCount; f++)
            {
                ...
                animTrack.PositionKeys.Add(new VectorKey(1.0 * f * anim.FrameDuration,
                    new Vector3D(rootMotionOnFrame.X * -100, rootMotionOnFrame.Y * 100,
                        rootMotionOnFrame.Z * 100)));
    ...
    public static string ExportToFile(HavokAnimationData anim, string filePath, string assimpFileFormatStr)
    {
